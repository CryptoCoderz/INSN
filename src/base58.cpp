...
namespace {
    class CINSaNeAddressVisitor : public boost::static_visitor<bool> {
    private:
        CINSaNeAddress *addr;
    public:
        CINSaNeAddressVisitor(CINSaNeAddress *addrIn) : addr(addrIn) { }
...
bool CINSaNeAddress::Set(const CKeyID &id) {
    SetData(Params().Base58Prefix(CChainParams::PUBKEY_ADDRESS), &id, 20);
    return true;
}

bool CINSaNeAddress::Set(const CScriptID &id) {
    SetData(Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS), &id, 20);
    return true;
}

bool CINSaNeAddress::Set(const CTxDestination &dest) {
    return boost::apply_visitor(CINSaNeAddressVisitor(this), dest);
}

bool CINSaNeAddress::IsValid() const {
...
CTxDestination CINSaNeAddress::Get() const {
    if (!IsValid())
...
bool CINSaNeAddress::GetKeyID(CKeyID &keyID) const {
...
bool CINSaNeAddress::IsScript() const {
    return IsValid() && vchVersion == Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS);
}

void CINSaNeSecret::SetKey(const CKey& vchSecret) {
...
CKey CINSaNeSecret::GetKey() {
    CKey ret;
    ret.Set(&vchData[0], &vchData[32], vchData.size() > 32 && vchData[32] == 1);
    return ret;
}

bool CINSaNeSecret::IsValid() const {
    bool fExpectedFormat = vchData.size() == 32 || (vchData.size() == 33 && vchData[32] == 1);
    bool fCorrectVersion = vchVersion == Params().Base58Prefix(CChainParams::SECRET_KEY);
    return fExpectedFormat && fCorrectVersion;
}

bool CINSaNeSecret::SetString(const char* pszSecret) {
    return CBase58Data::SetString(pszSecret) && IsValid();
}

bool CINSaNeSecret::SetString(const std::string& strSecret) {
