...
{
    // Convert INSaNe:// to INSaNe:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("INSaNe://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "INSaNe:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "INSaNe.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for INSaNe.lnk
...
{
    return GetAutostartDir() / "INSaNe.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a INSaNe.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=INSaNe\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the INSaNe app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add INSaNe app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("INSaNe-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  INSaNe-qt [" + tr("command-line options") + "]                     " + "\n";
