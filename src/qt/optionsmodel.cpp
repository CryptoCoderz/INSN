...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeINSaNeAmount"))
        settings.setValue("nAnonymizeINSaNeAmount", 1000);
    nAnonymizeINSaNeAmount = settings.value("nAnonymizeINSaNeAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeINSaNeAmount"))
        SoftSetArg("-anonymizeINSaNeamount", settings.value("nAnonymizeINSaNeAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeINSaNeAmount:
            return QVariant(nAnonymizeINSaNeAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeINSaNeAmount:
            nAnonymizeINSaNeAmount = value.toInt();
            settings.setValue("nAnonymizeINSaNeAmount", nAnonymizeINSaNeAmount);
            emit AnonymizeINSaNeAmountChanged(nAnonymizeINSaNeAmount);
            break;
        default:
            break;
        }
